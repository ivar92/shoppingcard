    ...
    <div class="sidebar">
        ...
        <div class="image">
          <img src="{{asset('backend/dist/img/user2-160x160.jpg')}}" class="img-circle elevation-2" alt="User Image">
            ...
            @if (Auth::check())
                <div class="info">
                    @php
                        $nom = auth()->administrators()->nom;
                        dd($nom); // affiche le contenu de $nom
                    @endphp
                    @if ($nom)
                        <a href="#" class="d-block">{{ $nom }}</a>
                    @else
                        <a href="#" class="d-block">User name not found</a>
                    @endif
                </div>
            @endif
        </div>
      ...
      <nav class="mt-2">
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          <li class="nav-item has-treeview {{request()->is('addproduct') ? 'menu-open' : ''}} {{request()->is('product') ? 'menu-open' : ''}}">
            <a href="#" class="nav-link {{request()->is('addproduct') ? 'active' : ''}} {{request()->is('product') ? 'active' : ''}}">
              <i class="nav-icon fas fa-folder"></i>
              <p>
                Products
                <i class="fas fa-angle-left right"></i>
          ...
          <li class="nav-item">
            <a href="{{('/disconnected')}}" class="nav-link">
